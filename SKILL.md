...
本技能依据公开海报结构重建，用于 Grok 生图或改图，不是对方付费 GPTs 原文件。

## 实拍得出的硬规则

对真人照片做 `render_edited_image` 时，模型会黏在原图比例上。只写「去皱、去油、大一点眼睛」会把系统修成精修证件照。因此：
...
- 有照片要保留这个人 → `render_edited_image`，**始终以用户原始照片为 image_id**，不要用上一张生成图当新底，除非用户明确说「就在这张上改」。
- 两张及以上参考做双人 → 同一规则，写清左右。
- 纯文字或演示脸 → `render_generated_image`。
- 只要提示词 → 不生图。
